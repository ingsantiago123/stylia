...
export async function recorrectMacro(
  docId: string
): Promise<{ message: string; task_id: string; macro_correction_level: string }> {
  const res = await fetch(`${API_BASE}/documents/${docId}/recorrect-macro`, {
    method: "POST",
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({ detail: "Error desconocido" }));
