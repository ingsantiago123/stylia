...
export function ProfileSelector({ docId, filename, onProcessStarted, onCancel }: ProfileSelectorProps) {
  ...
  return (
    <div className="bg-carbon-100 border border-carbon-300 rounded-xl p-6 space-y-5">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-bruma">Perfil editorial</h3>
        ...
        <button
          onClick={onCancel}
          className="text-plomo hover:text-bruma transition-colors p-2"
          title="Cancelar"
        >
      ...
      {/* Grid de presets */}
      {loading ? (
        <div className="text-plomo text-sm text-center py-8">Cargando perfiles...</div>
      ) : (
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${isSelected
                    ? "border-krypton bg-krypton/10 shadow-[0_0_20px_rgba(212,255,0,0.1)]"
                    : "border-carbon-300 hover:border-carbon-200 bg-carbon-50 hover:bg-carbon-100"
                  }
                  ${processing ? "opacity-50 cursor-wait" : "cursor-pointer"}
                `}
              >
                {/* Icon */}
                <div className={`mb-3 ${isSelected ? "text-krypton" : "text-plomo group-hover:text-bruma"} transition-colors`}>
                  {ICON_MAP[preset.icon] || ICON_MAP["file-text"]}
                </div>

                {/* Name */}
                <div className={`text-sm font-medium mb-1 ${isSelected ? "text-krypton" : "text-bruma"}`}>
                  {preset.name}
                </div>

                {/* Intervention level badge */}
                <div className={`text-xs ${INTERVENTION_COLORS[preset.intervention_level] || "text-plomo"}`}>
                  {INTERVENTION_LABELS[preset.intervention_level] || preset.intervention_level}
                </div>

                {/* Description tooltip on hover */}
                <div className="mt-2 text-xs text-plomo line-clamp-2 leading-relaxed">
                  {preset.description}
                </div>
                ...
                {isSelected && (
                  <div className="absolute top-2 right-2">
                    <svg className="w-5 h-5 text-krypton" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div className="flex items-center gap-3 pt-2">
        <button
          ...
          className={`
            ...
            ${selectedKey && !processing
              ...
              : "bg-carbon-200 text-plomo cursor-not-allowed"
            }
          `}
        >
          {processing ? (
        ...
        <button
          ...
          disabled={processing}
          className="px-4 py-2.5 rounded-lg text-sm font-medium text-plomo hover:text-bruma border border-carbon-300 hover:border-carbon-200 transition-all"
        >
          Personalizar
        </button>
        ...
        <button
          ...
          disabled={processing}
          className="px-4 py-2.5 rounded-lg text-sm text-plomo hover:text-bruma transition-colors ml-auto"
        >
          Sin perfil (generico)
        </button>
      </div>
